...
function Leads() {
  ...
  return (
    <div>
      <PageHeader title={t('Leads')} sub={`${activeLeads.length} ${t('active')}`} />
          ...
          return (
            <div key={status} className="w-64 shrink-0">
              <div className="mb-2 flex items-center justify-between px-1">
                <span className="text-xs font-bold tracking-wide text-[#1c1712]/60 uppercase">{t(COLUMN_LABELS[status])}</span>
                ...
                {leads.map((lead) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-0.5 text-xs text-[#1c1712]/55">{eventType(lead.eventType)}</div>
                      ...
                      >
                        {LEAD_STATUSES.map((s) => (
                          <option key={s} value={s}>{t(COLUMN_LABELS[s])}</option>
                        ))}
                      </select>
                    </Link>
                  )
                })}
                {leads.length === 0 && <div className="rounded-xl border border-dashed border-black/10 px-3 py-6 text-center text-xs text-[#1c1712]/35">{t('Empty')}</div>}
              </div>
            </div>
          )
        })}
      ...
      <div className="flex flex-col gap-6 px-6 py-6 md:hidden">
                ...
                {leads.map((lead) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-0.5 text-xs capitalize text-[#1c1712]/55">{eventType(lead.eventType)}</div>
                      <select
                        value={lead.status}
                        onChange={(e) => { e.stopPropagation(); updateLeadStatus(lead.id, e.target.value as LeadStatus) }}
                        onClick={(e) => e.preventDefault()}
        ...
        {activeLeads.length === 0 && <div className="py-10 text-center text-sm text-[#1c1712]/50">{t('No active leads.')}</div>}
      </div>

      {closedLeads.length > 0 && (
        <div className="border-t border-black/10 px-6 py-6">
          <h2 className="mb-3 text-xs font-bold tracking-wide text-[#1c1712]/60 uppercase">{t('Booked / completed / lost')}</h2>
          <div className="flex flex-col gap-1.5">
            {closedLeads.map((lead) => {
              ...
              return (
                ...
                >
                  <span className="font-semibold">{customer?.name}</span>
                  <span className="text-xs text-[#1c1712]/50">{t(COLUMN_LABELS[lead.status])}</span>
                </Link>
              )
            })}
          </div>
