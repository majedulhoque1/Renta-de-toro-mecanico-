...
function LeadDetail() {
  ...
  if (!lead) return <EmptyState text={t('Lead not found.')} />
  ...
  function handleConvert() {
    ...
    const { bookingId } = convertLeadToBooking(lead.id, {
      ...
      title: `${eventType(lead.eventType)} — ${customer?.name ?? ''}`,
    })
  ...
  return (
    <div>
      <PageHeader
        title={customer?.name ?? t('Lead')}
        sub={`${eventType(lead.eventType)} · ${lead.eventDate ?? t('no date')}`}
      ...
      <div className="grid grid-cols-1 gap-6 px-6 py-6 lg:grid-cols-[1fr_320px]">
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-xl border border-black/10 bg-white p-4">
            <h2 className="mb-3 text-sm font-bold">{t('Activity')}</h2>
            <div className="flex flex-col gap-3">
              {activities.map((a) => (
                <div key={a.id} className="border-l-2 border-black/10 pl-3 text-sm">
                  <div>{a.body}</div>
                  <div className="mt-0.5 text-xs text-[#1c1712]/45">{new Date(a.createdAt).toLocaleString(locale)}</div>
                </div>
              ))}
              {activities.length === 0 && <p className="text-sm text-[#1c1712]/50">{t('No activity yet.')}</p>}
            </div>
            <form
              onSubmit={(e) => { e.preventDefault(); if (note.trim()) { addLeadNote(lead.id, note.trim()); setNote('') } }}
              className="mt-4 flex gap-2"
            >
              <input value={note} onChange={(e) => setNote(e.target.value)} placeholder={t('Add a note…')} className="input flex-1" />
              <button type="submit" className="rounded-lg bg-[#1c1712] px-4 text-sm font-bold text-[#f4f1ea]">{t('Add')}</button>
            </form>
          </div>
            ...
            <div className="rounded-xl border border-black/10 bg-white p-4">
              {!showConvert ? (
                <button type="button" onClick={() => setShowConvert(true)} className="rounded-lg bg-[#b5701c] px-4 py-2.5 text-sm font-bold text-white">
                  {t('Convert to booking →')}
                </button>
              ) : (
                <div className="flex flex-col gap-3">
                  <h2 className="text-sm font-bold">{t('Convert to booking')}</h2>
                  <div className="grid grid-cols-2 gap-3">
                    <label className="text-xs font-bold">{t('Start time')}
                      <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="input mt-1" />
                    </label>
                    <label className="text-xs font-bold">{t('Duration (hours)')}
                      <input type="number" value={hours} onChange={(e) => setHours(e.target.value)} className="input mt-1" />
                    </label>
                    <label className="col-span-2 text-xs font-bold">{t('Location')}
                      <input value={location} onChange={(e) => setLocation(e.target.value)} className="input mt-1" />
                    </label>
                    <label className="col-span-2 text-xs font-bold">{t('Total ($)')}
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl border border-black/10 bg-white p-4 text-sm">
            <h2 className="mb-2 text-sm font-bold">{t('Contact')}</h2>
            <div className="flex flex-col gap-1 text-[#1c1712]/75">
              <span>{customer?.phone}</span>
              {customer?.email && <span>{customer.email}</span>}
              <span>{lead.city}</span>
              {lead.attendance && <span>{lead.attendance} {t('guests')}</span>}
            </div>
          </div>

          <div className="rounded-xl border border-black/10 bg-white p-4 text-sm">
            <h2 className="mb-2 text-sm font-bold">{t('Follow-up')}</h2>
            <input
              type="date"
          ...
          <div className="rounded-xl border border-black/10 bg-white p-4 text-sm">
            <h2 className="mb-2 text-sm font-bold">{t('Quote')}</h2>
            {lead.quoteAmount ? (
              ...
              <form onSubmit={(e) => { e.preventDefault(); if (quote) setLeadQuote(lead.id, Number(quote)) }} className="flex gap-2">
                ...
                <button type="submit" className="rounded-lg bg-[#1c1712] px-3 text-sm font-bold text-[#f4f1ea]">{t('Send')}</button>
              </form>
            )}
          </div>
