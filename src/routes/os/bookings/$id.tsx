...
function BookingDetail() {
  ...
  if (!booking) return <EmptyState text={t('Booking not found.')} />
  ...
  return (
    <div>
      <PageHeader
        title={booking.title || customer?.name || t('Booking')}
        sub={new Intl.DateTimeFormat(locale, { weekday: 'long', month: 'long', day: 'numeric' }).format(new Date(booking.startAt))}
      ...
      <div className="grid grid-cols-1 gap-6 px-6 py-6 lg:grid-cols-[1fr_320px]">
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-xl border border-black/10 bg-white p-4">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-sm font-bold">{t('Checklist')}</h2>
            ...
            {(['before', 'event_day', 'after'] as const).map((phase) => (
              <div key={phase} className="mb-4 last:mb-0">
                <h3 className="mb-1.5 text-xs font-bold tracking-wide text-[#1c1712]/50 uppercase">{t(PHASE_LABELS[phase])}</h3>
                <div className="flex flex-col gap-1">
                  {checklist.filter((i) => i.phase === phase).map((item) => (
                    <label key={item.id} className="flex items-center gap-2 rounded-lg px-1.5 py-1 text-sm hover:bg-black/[0.02]">
                      ...
                      <span className={item.done ? 'text-[#1c1712]/40 line-through' : ''}>{t(item.label)}</span>
                    </label>
                  ))}
                </div>
              </div>
          ...
          {/* Payments */}
          <div className="rounded-xl border border-black/10 bg-white p-4">
            <h2 className="mb-3 text-sm font-bold">{t('Payments')}</h2>
            <div className="mb-3 flex flex-col gap-1">
              {payments.map((p) => (
                <div key={p.id} className="flex items-center justify-between text-sm">
                  <span className="capitalize text-[#1c1712]/70">{t(cap(p.kind))} · {t(cap(p.method))}</span>
                  <span className="font-semibold tabular-nums">${p.amount}</span>
                </div>
              ))}
              {payments.length === 0 && <p className="text-sm text-[#1c1712]/50">{t('No payments recorded.')}</p>}
            </div>
            <form
            ...
            >
              <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder={t('Amount')} className="input" />
              ...
              <button type="submit" className="rounded-lg bg-[#1c1712] px-3 text-sm font-bold text-[#f4f1ea]">{t('Record')}</button>
            </form>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl border border-black/10 bg-white p-4 text-sm">
            <h2 className="mb-2 text-sm font-bold">{t('Event')}</h2>
            <div className="flex flex-col gap-1 text-[#1c1712]/75">
              <span>{customer?.name} · {customer?.phone}</span>
              <span>{booking.location}</span>
              {booking.attendance && <span>{booking.attendance} {t('guests')}</span>}
              <span className="capitalize">{eventType(booking.eventType)}</span>
            </div>
          ...
          <div className="rounded-xl border border-black/10 bg-white p-4 text-sm">
            <h2 className="mb-2 text-sm font-bold">{t('Financials')}</h2>
            <div className="flex justify-between"><span className="text-[#1c1712]/60">{t('Total')}</span><span className="font-semibold">${booking.total}</span></div>
            <div className="flex justify-between"><span className="text-[#1c1712]/60">{t('Deposit required')}</span><span className="font-semibold">${booking.depositRequired}</span></div>
            <div className="flex justify-between"><span className="text-[#1c1712]/60">{t('Paid')}</span><span className="font-semibold">${paidTotal}</span></div>
            <div className="flex justify-between border-t border-black/10 pt-1.5 mt-1.5"><span className="text-[#1c1712]/60">{t('Balance')}</span><span className="font-bold">${balance}</span></div>
            <div className="mt-2"><StatusBadge status={payStatus} /></div>
          </div>

          <div className="rounded-xl border border-black/10 bg-white p-4 text-sm">
            <h2 className="mb-2 text-sm font-bold">{t('Staff')}</h2>
            {booking.staffName ? (
              ...
              <form onSubmit={(e) => { e.preventDefault(); if (staff.trim()) assignStaff(booking.id, staff.trim()) }} className="flex gap-2">
                <input value={staff} onChange={(e) => setStaff(e.target.value)} placeholder={t('Name')} className="input flex-1" />
                <button type="submit" className="rounded-lg bg-[#1c1712] px-3 text-sm font-bold text-[#f4f1ea]">{t('Assign')}</button>
              </form>
            )}
          </div>
