    ...
    <div>
      ...
      <div className="flex max-w-xl flex-col gap-6 px-6 py-6">
        <div className="rounded-xl border border-black/10 bg-white p-4">
          <h2 className="mb-3 text-sm font-bold">{t('Brand')}</h2>
          ...
          <p className="mt-2 text-xs text-[#1c1712]/50">{t('Shown on the public site header/footer. Currently a working name, pending confirmation.')}</p>
        </div>

        <div className="rounded-xl border border-black/10 bg-white p-4">
          <h2 className="mb-3 text-sm font-bold">{t('Starting prices')}</h2>
          <p className="mb-3 text-xs text-[#1c1712]/50">
            {t('Leave blank to show "Cotización gratis / Free quote" on the public packages section instead of a price.')}
          </p>
          <div className="flex flex-col gap-2">
            {EVENT_TYPE_KEYS.map((key) => (
              <div key={key} className="flex items-center justify-between gap-3">
                <span className="text-sm">{(lang === 'es' ? es : en).wizard.eventTypeLabels[key]}</span>
        ...
        <div className="rounded-xl border border-black/10 bg-white p-4">
          <h2 className="mb-3 text-sm font-bold">{t('Deposit')}</h2>
          <label className="block text-xs font-bold">
            {t('Deposit required at booking (%)')}
            <input
              type="number"
              min={0}
              max={100}
        ...
        <div className="rounded-xl border border-red-200 bg-red-50 p-4">
          <h2 className="mb-2 text-sm font-bold text-red-900">{t('Reset demo data')}</h2>
          <p className="mb-3 text-xs text-red-800/70">{t('Restores the original seeded leads, bookings and payments. Cannot be undone.')}</p>
          {!confirmReset ? (
